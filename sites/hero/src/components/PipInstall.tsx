...
export function PipInstall({ size = "big", className = "" }: Props) {
  ...
  if (size === "chip") {
    return (
      <button
        onClick={copy}
        className={`group inline-flex items-center gap-2.5 rounded-full border border-cyan/30 bg-panel px-4 py-1.5 font-mono text-[13px] text-ice/90 transition-colors hover:border-cyan/70 ${className}`}
  ...
  return (
    <button
      ...
      className={`group inline-flex items-center rounded-2xl border border-cyan/25 bg-panel text-left transition-colors hover:border-cyan/60 ${
        isMega ? "gap-5 px-7 py-6 sm:gap-7 sm:px-10 sm:py-8" : "gap-4 px-6 py-4 sm:px-7 sm:py-5"
      } ${className}`}
      aria-label="Copy pip install burla"
      style={{
        boxShadow:
