...
export function Nav() {
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[1120px] px-5 md:px-8 flex items-center justify-between h-[68px]">
        <Logo />
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            ...
            <span className="mono normal-case tracking-normal">github</span>
      ...
      >
        <div className="mx-auto w-full max-w-[1120px] px-5 md:px-8 pb-6 pt-2 flex flex-col gap-1">
          {SECTIONS.map((s) => (
            <button
              key={s.id}
              type="button"
              onClick={() => goToSection(s.id)}
              className="py-3 text-left text-[17px] font-medium text-ink border-b border-line"
            >
              {s.label}
            </button>
          ))}
